...
function calculateTotalSalaries() {
    const totalSalaries = employees.reduce((totalSalaries, currentEmployee) => totalSalaries + currentEmployee.salary, 0);
...
function findEmployeeBySpecialization(employeeSpecialization) {
    const foundEmployee = employees.find(employee => employee.speciaization === employeeSpecialization);
    if (foundEmployee) 
    {
    ...
    {
        document.getElementById('employeesDetails').innerHTML = 'no employee has been found with this Specialization -___-|||';
    }
}
